...
* **Client**: AngularJS and Twitter Bootstrap with pure html partials (no server side rendering so it's full static and CDN ready). Bower packages are located at `client\bower_packages`.
...
## Who uses it
We're currently using KOAN at Cryptzone for the frontend development of our next-generation enterprise cloud platform. It is also used by NBusy, a small startup aiming to create an innovative social sharing platform. KOAN will be developed and maintained for years to come, and evolve along with underlying technology.
